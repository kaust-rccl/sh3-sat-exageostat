 ...
 * */
	...
	MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC, msequence, mrequest, &data->l1, &data->l1, initial_theta, data->dm, data->c_fun);
	VERBOSE(" Done.\n");
	...
	*/
	...
	if(MORSE_My_Mpi_Rank() == 0)
	{
	        ...
	        printf(" %3d- Model Parameters (varinace, range, smoothness): (%2.6f, %2.6f, %2.6f) ----> LogLi: %2.6f\n",
			 ((MLE_data*)data)->iter_count+1,  data->variance, theta[1], theta[2], loglik);
 ...
 * */
{
	...
	MORSE_MLE_dcmg_Tile_Async(MorseLower, MORSE_descC12, hsequence, hrequest,  &data->lmiss, &data->lobs, theta, data->dm, data->c_fun);
	MORSE_Sequence_Wait(hsequence);
