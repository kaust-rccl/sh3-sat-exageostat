...
int main(int argc, char **argv) {
	...
	//Parse inputs
	theta_parser2(initial_theta, theta, 3);   
	theta_parser2(lb, clb, 3);
	theta_parser2(ub, cub, 3);



	rexageostat_init(&ncores,&gpus, &dts);
	gen_z_exact(&n,   &ncores,   &gpus,
			&dts,   &p_grid,  &q_grid,
			&initial_theta[0],  &initial_theta[1],  &initial_theta[2], 
			&dm_int, &seed,  &globalveclen, vecs_out);

	if(strcmp (computation, "exact") == 0)
		mle_exact(&n, &ncores, &gpus,
				&dts, &p_grid, &q_grid,  
				vecs_out, NULL,  &vecs_out[n],
