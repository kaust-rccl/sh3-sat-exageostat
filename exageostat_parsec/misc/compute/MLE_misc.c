...
void zsort_locations(int n, location * locations)
...
void zsort_locations_3d(int n, location * locations)
	//! Sort in Morton order (input points must be in [0;1]x[0;1] square])
{
	// Some sorting, required by spatial statistics code
	int i;
	 ...
	 * */
{
	...
	for(i = 0; i< count; i++)
	{
		...
		z[i] = new_z[i];
//		printf("%f - %f - %f - %f\n",locations->x[i], locations->y[i], locations->z[i], z[i]);
...
void write_to_thetafile(char * path, double theta0,double theta1, double theta2, double theta3, int n, double time_per_iter, int total_no_iters, double prediction_error)
	...
	//! shuffle an array
{
	if (n > 1) 
	{
		...
		for (i = 0; i < n - 1; i++) 
		{
			...
			if(locations->z != NULL)
			{
				double ztemp = locations->z[j];
				locations->z[j] = locations->z[i];
				locations->z[i] = ztemp;
			}
...
void theta_parser(double *initial_theta, double *target_theta, double *starting_theta, char *ikernel, char *kernel, double *lb, double *up, int test, int num_params)
...
void print_result(MLE_data *data, double *starting_theta, int N, int zvecs, int ncores, int ts, int test, double *initial_theta, char *computation, int p_grid, int q_grid, double final_loglik, double prediction_error)
//! print results (only for testing accuracy should be removed in release)
{
	if ( 0 == data->rank )
	{
		...
		{
			sprintf(buf, "%d-%d-", data->band_size_double, data->band_size_single);
			strcat(str, buf);
			//strcat(str, "-0-0-");
		}
		...
		{
			sprintf(buf, "%0.3f-", initial_theta[0]);
			strcat(str, buf);
			sprintf(buf, "%0.3f-", initial_theta[1]);
			strcat(str, buf);
			sprintf(buf, "%.3f-", initial_theta[2]);
			strcat(str, buf);
		}
		else
		{		
		...
		write_to_thetafile(str, data->variance, starting_theta[1], starting_theta[2], starting_theta[3], zvecs, data->avg_exec_time_per_iter,data->iter_count, prediction_error );
	}
}
...
void pick_random_points(MLE_data *data, double *Zobs, double *Zactual, int nZmiss, int nZobs, int N)
{
	...
	location *lobs;
	double *Z;
        void MLE_get_zobs(MLE_data *,double*,int);
	int i = 0;
	...
	//! write prediction results (only for testing accuracy should be removed in release).
{
	...
	{
		fprintf(pFile, "%.17lf ", theta[i]);
	}
	fprintf(pFile, "%.17lf ", loglik);
