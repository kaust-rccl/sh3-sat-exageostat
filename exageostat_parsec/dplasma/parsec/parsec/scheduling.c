...
parsec_context_t* parsec_context = es->virtual_process->parsec_context;
    //fprintf(stderr,"%lld: __parsec_task_progress\n",pthread_self());
    ...
    case PARSEC_HOOK_RETURN_DONE: {
        if(task->status <= PARSEC_TASK_STATUS_HOOK) {
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_execute - beg %d\n",pthread_self(),parsec_context->active_taskpools);
            rc = __parsec_execute( es, task );
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_execute - end %d\n",pthread_self(),parsec_context->active_taskpools);
        }
        /* We're good to go ... */
        switch(rc) {
        case PARSEC_HOOK_RETURN_DONE:    /* This execution succeeded */
            task->status = PARSEC_TASK_STATUS_COMPLETE;
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_complete_execution - beg %d\n",pthread_self(),parsec_context->active_taskpools);
            __parsec_complete_execution( es, task );
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_complete_execution - end %d\n",pthread_self(),parsec_context->active_taskpools);
            break;
        case PARSEC_HOOK_RETURN_AGAIN:   /* Reschedule later */
            ...
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_schedule - beg\n",pthread_self());
            __parsec_schedule(es, task, distance + 1);
            //fprintf(stderr,"%lld: __parsec_task_progress __parsec_schedule - end\n",pthread_self());
            break;
    ...
    case PARSEC_HOOK_RETURN_AGAIN:   /* Reschedule later */
        if(0 == task->priority) {
            ...
            task->priority /= 10;  /* demote the task */
        PARSEC_LIST_ITEM_SINGLETON(task);
        //fprintf(stderr,"%lld: __parsec_task_progress __parsec_schedule2 - beg\n",pthread_self());
        __parsec_schedule(es, task, distance + 1);
        //fprintf(stderr,"%lld: __parsec_task_progress __parsec_schedule2 - end\n",pthread_self());
        break;
    default:
    ...
    //fprintf(stderr,"%lld: __parsec_task_progress leaving\n",pthread_self());
    return rc;
}
...
int __parsec_context_wait( parsec_execution_stream_t* es )
{
    ...
    //fprintf(stderr,"%lld: __parsec_context_wait before wait_for_the_next_round\n",pthread_self());
  ...
  wait_for_the_next_round:
    ...
    parsec_barrier_wait( &(parsec_context->barrier) );
    //fprintf(stderr,"%lld: __parsec_context_wait all threads here and master signalled\n",pthread_self());
  ...
  skip_first_barrier:
    //fprintf(stderr,"%lld: all_tasks_done(%p)\n",pthread_self(),parsec_context);
    while( !all_tasks_done(parsec_context) ) {

        //fprintf(stderr,"%lld: active_taskpools (%d)\n",pthread_self(),parsec_context->active_taskpools);
        if(PARSEC_THREAD_IS_MASTER(es)) {
        ...
        if( (1 == parsec_communication_engine_up) &&
            ...
            //fprintf(stderr,"%lld: MASTER wait for remote deps\n",pthread_self());
        ...
        if( task != NULL ) {
            ...
            //fprintf(stderr,"%lld: going to task_progress\n",pthread_self(),parsec_context->active_taskpools);
            rc = __parsec_task_progress(es, task, distance);
            //fprintf(stderr,"%lld: after task_progress active_taskpools (%d)\n",pthread_self(),parsec_context->active_taskpools);
...
int parsec_context_start( parsec_context_t* context )
{
    ...
    if( __parsec_context_cas_or_flag(context, PARSEC_CONTEXT_FLAG_COMM_ACTIVE) ) {
        ...
        //fprintf(stderr,"%lld: wake up the other threads\n",pthread_self());
...
int parsec_context_wait( parsec_context_t* context )
{
    ...
    if( __parsec_context_cas_or_flag(context,
        ...
        //fprintf(stderr,"%lld: before parsec_remote_dep_on\n",pthread_self());
        (void)parsec_remote_dep_on(context);
    }
    ...
    //fprintf(stderr,"%lld: before __parsec_context_wait %d\n",pthread_self(),context->virtual_processes[0]->execution_streams[0]);
    ret = __parsec_context_wait( context->virtual_processes[0]->execution_streams[0] );
    //fprintf(stderr,"%lld: after  __parsec_context_wait %d\n",pthread_self(),context->virtual_processes[0]->execution_streams[0]);
