...
module load mpich/3.3/gnu6.4.0-cuda9.2
...
then
    ...
    then
        ...
        make -j PREFIX=$PWD/openblas_install >/dev/null|| make PREFIX=$PWD/openblas_install || { echo 'OpenBLAS installation failed' ; exit 1; }
...
then
    ...
    then
        echo "Building Hwloc..."
        cd $SETUP_DIR
        wget https://download.open-mpi.org/release/hwloc/v1.11/hwloc-1.11.5.tar.gz -O - | tar -zx
    cd hwloc-1.11.5
    [[ -d hwloc_install ]] || mkdir hwloc_install
        ./configure --prefix=$PWD/hwloc_install
...
then
    ...
    cd $SETUP_DIR
        wget http://starpu.gforge.inria.fr/files/starpu-1.2.6/starpu-1.2.6.tar.gz -O - | tar -zx
        cd starpu-1.2.6
...
then
    ...
    then
        ...
        rm -rf ./CMake*
        cmake -DCHAMELEON_USE_MPI=$MPIVALUE -DCMAKE_BUILD_TYPE="Release" -DCMAKE_C_FLAGS_RELEASE="-O3 -Ofast -w" -DCHAMELEON_USE_CUDA=$CUDAVALUE -DCHAMELEON_ENABLE_EXAMPLE=OFF -DCHAMELEON_ENABLE_TESTING=OFF -DCHAMELEON_ENABLE_TIMING=OFF -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=$PWD/install_dir  ..
...
then
    ...
    rm -rf build
        mkdir -p build/install_dir && cd build
        rm -rf ./CMake*
        cmake -DHICMA_USE_MPI=$MPIVALUE -DCMAKE_BUILD_TYPE="Release" -DCMAKE_C_FLAGS_RELEASE="-O3 -Ofast -w" -DBUILD_SHARED_LIBS=ON -DHICMA_ENABLE_TESTING=OFF -DHICMA_ENABLE_TIMING=OFF -DCMAKE_INSTALL_PREFIX=$PWD/install_dir   ..
...
then
    ...
    rm -rf build
        mkdir -p build/install_dir && cd build
        rm -rf ./CMake*
     cmake -DCMAKE_BUILD_TYPE="Release" -DCMAKE_C_FLAGS_RELEASE="-O3 -Ofast -w" -DBUILD_SHARED_LIBS=ON -DEXAGEOSTAT_EXAMPLES=OFF   -DEXAGEOSTAT_USE_MPI=$MPIVALUE -DEXAGEOSTAT_USE_HICMA=ON -DCMAKE_INSTALL_PREFIX=$PWD/install_dir ..
