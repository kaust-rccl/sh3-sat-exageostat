...
void core_dcmg (double *A, int m, int n,
	...
	else
	{
		for (i = 0; i < m; i++) {
			...
			z0 = l1->z[i0];
			for (j = 0; j < n; j++) {
				expr = calculateDistance3d(x0, y0, z0, l2->x[j0], 
						l2->y[j0], l2->z[j0], distance_metric)/localtheta[1];
				if(expr == 0)
				{
					A[i + j * m] = sigma_square;// +tmp; /*+ 1e-4*/;
				}
		...
		for (i = 0; i < m; i++) {
			...
			for (j = 0; j < n; j++) {
				expr = calculateDistance3d(x0, y0, z0, 
						l2->x[j0], l2->y[j0], l2->z[j0], distance_metric)/localtheta[1];
				if(expr == 0)
					A[i + j * m] = sigma_square /*+ 1e-4*/;
				else
					A[i + j * m] = con*pow(expr, localtheta[2])*gsl_sf_bessel_Knu(localtheta[2],expr); // Matern Function
...
//*****************************************************************
void core_dcmg_pow_exp_nuggets(double *A, int m, int n,
                int m0, int n0,
                location  *l1, location *l2,
                double *localtheta, int distance_metric) {
        ...
        if(l1->z == NULL || l2->z == NULL )
        {
                for (i = 0; i < m; i++) {
                        ...
                        for (j = 0; j < n; j++) {
                                ...
                                if(expr == 0)
                                {
                                        A[i + j * m] = sigma_square + localtheta[3];//+tmp; /*+ 1e-4*/;
                                }
                                else
                                        A[i + j * m] = sigma_square *  exp(-(expr1/localtheta[1])) ; // power-exp kernel
                                j0++;
...
void core_scmg_pow_exp (float *A, int m, int n,
	...
	if(l1->z == NULL || l2->z == NULL )
	{
		...
		for (i = 0; i < m; i++) {
			...
			y0 = l1->y[i0];
			for (j = 0; j < n; j++) {
				expr  = calculateDistance(x0, y0, 
						l2->x[j0], l2->y[j0], distance_metric);
				expr1 = pow(expr, localtheta[2]);
				if(expr == 0)
					A[i + j * m] = sigma_square /*+ 1e-4*/;
				else
