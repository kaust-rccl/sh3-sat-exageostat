...
#include "hsdpotrf_L_trsm_convert.h"
...
static float evaluate_gpu_potrf_convert(parsec_task_t* task) {
    int my_indicator = ((__parsec_hsdpotrf_L_trsm_convert_potrf_hsdpotrf_convert_task_t *)task)->locals.my_indicator.value;
    if( my_indicator )
...
static float evaluate_gpu_trsm_convert(parsec_task_t* task) {
    int my_indicator = ((__parsec_hsdpotrf_L_trsm_convert_potrf_hsdtrsm_convert_task_t *)task)->locals.my_indicator.value;
    if( my_indicator )
        return PARSEC_HOOK_RETURN_DONE;
    else
        return PARSEC_HOOK_RETURN_NEXT;
...
static float evaluate_gpu_trsm_convert_half(parsec_task_t* task) {
    int my_indicator = ((__parsec_hsdpotrf_L_trsm_convert_potrf_hsdtrsm_convert_half_task_t *)task)->locals.my_indicator.value;
    if( my_indicator )
        return PARSEC_HOOK_RETURN_DONE;
    else
        return PARSEC_HOOK_RETURN_NEXT;
 ...
 ******************************************************************************/
parsec_taskpool_t*
dplasma_hsdpotrf_trsm_convert_New( parsec_context_t *parsec,
                    PLASMA_enum uplo,
...
{
    parsec_hsdpotrf_L_trsm_convert_taskpool_t *parsec_dpotrf = NULL;
    parsec_taskpool_t *tp = NULL;

    /* Check input arguments */
    if( uplo != PlasmaLower ) {
        dplasma_error("dplasma_hsdpotrf_trsm_convert_New", "illegal value of uplo");
        return NULL /*-1*/;
    }
    ...
    *info = 0;
    tp = (parsec_taskpool_t*)parsec_hsdpotrf_L_trsm_convert_new( uplo, A, info );

    parsec_dpotrf = (parsec_hsdpotrf_L_trsm_convert_taskpool_t*)tp;
...
*/
	...
	void** eval_gpu_trsm_convert_half;
	eval_gpu_trsm_convert_half = (void *)&parsec_dpotrf->super.task_classes_array[5]->incarnations[0].evaluate;
	*eval_gpu_trsm_convert_half = &evaluate_gpu_trsm_convert_half;
#endif

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_DOUBLE_ARENA],
                            parsec_datatype_double_t, matrix_UpperLower,
                            1, A->mb, A->nb, A->mb,
                            PARSEC_ARENA_ALIGNMENT_SSE, -1 );

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_SINGLE_ARENA],
                            parsec_datatype_float_t, matrix_UpperLower,
                            1, A->mb, A->nb, A->mb,
                            PARSEC_ARENA_ALIGNMENT_SSE, -1 );

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_HALF_ARENA],
 ...
 ******************************************************************************/
void
dplasma_hsdpotrf_trsm_convert_Destruct( parsec_taskpool_t *tp )
{
    parsec_hsdpotrf_L_trsm_convert_taskpool_t *parsec_dpotrf = (parsec_hsdpotrf_L_trsm_convert_taskpool_t *)tp;
    ...
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_DOUBLE_ARENA] );
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_SINGLE_ARENA] );
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_trsm_convert_HALF_ARENA] );
 ...
 ******************************************************************************/
int
dplasma_hsdpotrf_trsm_convert( parsec_context_t *parsec,
                PLASMA_enum uplo,
                parsec_tiled_matrix_dc_t *A,
                int lookahead, int send_less, int tensor_gemm,
                int band_size_double, int band_size_single, int HNB )
{
    ...
    parsec_dpotrf = dplasma_hsdpotrf_trsm_convert_New( parsec, uplo, A, &info, lookahead, send_less,
    ...
    if ( parsec_dpotrf != NULL )
    {
        ...
        dplasma_hsdpotrf_trsm_convert_Destruct( parsec_dpotrf );
