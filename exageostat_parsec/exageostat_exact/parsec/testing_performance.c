...
int main(int argc, char **argv) {
	...
	if( 0 == rank) fprintf(stderr, "00000000\n");

        /* stop gsl error handler */
        gsl_set_error_handler_off();

	/* Mix precision */
	if( 1 == kernel_type || 4 == kernel_type || 5 == kernel_type ) {
			/* Timer start */
			SYNC_TIME_START();
			/* Allocate memory */
			//parsec_band_allocate( parsec, (parsec_tiled_matrix_dc_t *)&dcC, band_size_double );
                        SYNC_TIME_PRINT(rank, ESC("Matrix_allocation" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d "
			...
			op_args->band_size_double = band_size_double;
			...
			/* Timer start */
			SYNC_TIME_START();
			//parsec_my_warmup( parsec, (parsec_tiled_matrix_dc_t *)&dcC);
			SYNC_TIME_PRINT(rank, ESC("Warmup_copy" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d "
						"band_size_double %d band_size_single %d lookahead %d ngpus %d: %lf\n",
						p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
						band_size_single, lookahead, num_gpus, sync_time_elapsed));
			...
			SYNC_TIME_START();
			if( 5 == kernel_type ) {
				info = dplasma_hsdpotrf_4taskclass( parsec, uplo, (parsec_tiled_matrix_dc_t *)&dcC,
						lookahead, sendless, tensor_gemm,
						band_size_double, band_size_single, HNB);
			} else if( 4 == kernel_type ) {
				info = dplasma_hsdpotrf_trsm_convert( parsec, uplo, (parsec_tiled_matrix_dc_t *)&dcC,
						lookahead, sendless, tensor_gemm,
						band_size_double, band_size_single, HNB);
			} else {
				info = dplasma_hsdpotrf( parsec, uplo, (parsec_tiled_matrix_dc_t *)&dcC,
						lookahead, sendless, tensor_gemm,
						band_size_double, band_size_single, HNB);
			}
        ...
        if( 2 == kernel_type ) {
			...
			dplasma_dplgsy( parsec, (double)(N), uplo,
					(parsec_tiled_matrix_dc_t *)&dcC, random_seed);

                        /* Init C0 */
                        dplasma_dplgsy( parsec, (double)(dcC0.super.ln), uplo,
                                        (parsec_tiled_matrix_dc_t *)&dcC0, random_seed);
#endif
			SYNC_TIME_PRINT(rank, ESC("Matrix_generation_Double" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d "
                                        "band_size_double %d band_size_single %d lookahead %d ngpus %d: %lf\n",
                                        p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
                                        band_size_single, lookahead, num_gpus, sync_time_elapsed));
	...
	if( 3 == kernel_type ) {
			...
			band_size_single = NT;
                        ...
                        SYNC_TIME_START();
                        /* Allocate memory */
                        //parsec_band_allocate( parsec, (parsec_tiled_matrix_dc_t *)&dcC, 0 );
                        SYNC_TIME_PRINT(rank, ESC("Matrix_allocation" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d "
                                                "band_size_double %d band_size_single %d lookahead %d ngpus %d: %lf\n",
                                                p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
                                                band_size_single, lookahead, num_gpus, sync_time_elapsed));
			...
			SYNC_TIME_START();
#if MATRIX_GENERATION
			parsec_smatrix_generation( parsec, (parsec_tiled_matrix_dc_t *)&dcC, &data.l1, &data.l1,
					initial_theta , data.dm, data.c_fun, arguments.band_size_double );
#else
                        matrix_init_args_t *op_args = (matrix_init_args_t *)malloc( sizeof(matrix_init_args_t) );
                        op_args->bump = (double)N;
                        ...
                        dplasma_dplgsy( parsec, (double)(dcC0.super.ln), uplo,
                                        (parsec_tiled_matrix_dc_t *)&dcC0, random_seed);
#endif
			SYNC_TIME_PRINT(rank, ESC("Matrix_generation_Single" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d "
						"band_size_double %d band_size_single %d lookahead %d ngpus %d: %lf\n",
						p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
						band_size_single, lookahead, num_gpus, sync_time_elapsed));
			...
			/* Timer start */
			SYNC_TIME_START();
			info = parsec_spotrf( parsec, uplo, (parsec_tiled_matrix_dc_t *)&dcC );
			SYNC_TIME_PRINT(rank, ESC("Potrf_Single" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d band_size_double %d "
						"band_size_single %d lookahead %d tensor_gemm %d ngpus %d: %lf\n",
						p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
						band_size_single, lookahead, tensor_gemm, num_gpus, (flops/1e9)/sync_time_elapsed));
			/* Record time */
			time_spotrf = sync_time_elapsed;
	...
	if( 6 == kernel_type ) {
			...
			/* Timer start */
			SYNC_TIME_START();
                        int parsec_shpotrf( parsec_context_t *, PLASMA_enum , parsec_tiled_matrix_dc_t *, int , int , int );

			info = parsec_shpotrf( parsec, uplo, (parsec_tiled_matrix_dc_t *)&dcC, band_size_double, tensor_gemm, lookahead );
			SYNC_TIME_PRINT(rank, ESC("Potrf_single_half" "\tPxQ= %3d %-3d NB= %4d N= %7d HNB= %d band_size_double %d "
						"band_size_single %d lookahead %d tensor_gemm %d ngpus %d: %lf\n",
						p_grid, nodes/p_grid, dts, N, HNB, band_size_double,
						band_size_single, lookahead, tensor_gemm, num_gpus, (flops/1e9)/sync_time_elapsed));
			/* Record time */
