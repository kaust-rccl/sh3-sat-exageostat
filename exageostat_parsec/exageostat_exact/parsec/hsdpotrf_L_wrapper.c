...
/*
static float evaluate_gpu_potrf_convert(parsec_task_t* task) {
    int my_indicator = ((__parsec_hsdpotrf_L_potrf_hsdpotrf_convert_task_t *)task)->locals.my_indicator.value;
    if( my_indicator )
...
static float evaluate_gpu_trsm_convert(parsec_task_t* task) {
    int my_indicator = ((__parsec_hsdpotrf_L_potrf_hsdtrsm_convert_task_t *)task)->locals.my_indicator.value;
    if( my_indicator )
        return PARSEC_HOOK_RETURN_DONE;
    else
        return PARSEC_HOOK_RETURN_NEXT;
 ...
 ******************************************************************************/
parsec_taskpool_t*
dplasma_hsdpotrf_New( parsec_context_t *parsec,
                    PLASMA_enum uplo,
...
{
    parsec_hsdpotrf_L_taskpool_t *parsec_dpotrf = NULL;
    parsec_taskpool_t *tp = NULL;

    /* Check input arguments */
    if ((uplo != PlasmaUpper) && (uplo != PlasmaLower)) {
        dplasma_error("dplasma_hsdpotrf_New", "illegal value of uplo");
        return NULL /*-1*/;
    }
    ...
    *info = 0;
    tp = (parsec_taskpool_t*)parsec_hsdpotrf_L_new( uplo, A, info );

    parsec_dpotrf = (parsec_hsdpotrf_L_taskpool_t*)tp;
...
/*
        void** eval_gpu_potrf_convert;
        eval_gpu_potrf_convert = (void *)&parsec_dpotrf->super.task_classes_array[2]->incarnations[0].evaluate;
        *eval_gpu_potrf_convert = &evaluate_gpu_potrf_convert;

        void** eval_gpu_trsm_convert;
        eval_gpu_trsm_convert = (void *)&parsec_dpotrf->super.task_classes_array[4]->incarnations[0].evaluate;
        *eval_gpu_trsm_convert = &evaluate_gpu_trsm_convert;
*/
#endif

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_DOUBLE_ARENA],
                            parsec_datatype_double_t, matrix_UpperLower,
                            1, A->mb, A->nb, A->mb,
                            PARSEC_ARENA_ALIGNMENT_SSE, -1 );

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_SINGLE_ARENA],
                            parsec_datatype_float_t, matrix_UpperLower,
                            1, A->mb, A->nb, A->mb,
                            PARSEC_ARENA_ALIGNMENT_SSE, -1 );

    parsec_matrix_add2arena(parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_HALF_ARENA],
 ...
 ******************************************************************************/
void
dplasma_hsdpotrf_Destruct( parsec_taskpool_t *tp )
{
    parsec_hsdpotrf_L_taskpool_t *parsec_dpotrf = (parsec_hsdpotrf_L_taskpool_t *)tp;
    ...
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_DOUBLE_ARENA] );
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_SINGLE_ARENA] );
    parsec_matrix_del2arena( parsec_dpotrf->arenas[PARSEC_hsdpotrf_L_HALF_ARENA] );
 ...
 ******************************************************************************/
int
dplasma_hsdpotrf( parsec_context_t *parsec,
                PLASMA_enum uplo,
                parsec_tiled_matrix_dc_t *A,
                int lookahead, int send_less, int tensor_gemm,
                int band_size_double, int band_size_single, int HNB )
{
    ...
    parsec_dpotrf = dplasma_hsdpotrf_New( parsec, uplo, A, &info, lookahead, send_less,
		    tensor_gemm, band_size_double, band_size_single, HNB );
    //fprintf(stderr,"parsec_dpotrf taskpool created %p\n",parsec_dpotrf);

    if ( parsec_dpotrf != NULL )
    {
        //fprintf(stderr,"adding the taskpool %p to the parsec context %p\n",parsec_dpotrf,parsec);
        parsec_context_add_taskpool( parsec, (parsec_taskpool_t*)parsec_dpotrf);
        //fprintf(stderr,"kick-off the parsec context %p\n",parsec);
        parsec_context_start(parsec);
        //fprintf(stderr,"wait for the parsec context %p\n",parsec);
        //sleep(10);fprintf(stderr,"1 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"2 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"3 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"4 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"5 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"6 parsec_context_test()=%d\n",parsec_context_test(parsec));
        //sleep(10);fprintf(stderr,"7 parsec_context_test()=%d\n",parsec_context_test(parsec));
        parsec_context_wait(parsec);
        //fprintf(stderr,"done waiting for the parsec context %p\n",parsec);
        dplasma_hsdpotrf_Destruct( parsec_dpotrf );
        //fprintf(stderr,"destroying the taskpool %p\n",parsec_dpotrf);
