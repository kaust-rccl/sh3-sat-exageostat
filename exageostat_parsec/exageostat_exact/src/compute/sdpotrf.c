...
int MORSE_sdpotrf(MORSE_enum uplo, int N,
                  double *A, double *temp, int LDA, int diag_thick)
{
    int NB;
    int status;
...
/* Quick return */
    ...
    MORSE_sdpotrf_Tile_Async(uplo, &descAt, &temp, diag_thick, sequence, &request);
 ...
 *
 ******************************************************************************/
int MORSE_sdpotrf_Tile(MORSE_enum uplo, MORSE_desc_t *A, MORSE_desc_t *temp, int diag_thick)
{
    MORSE_context_t *morse;
    MORSE_sequence_t *sequence = NULL;
    MORSE_request_t request = MORSE_REQUEST_INITIALIZER;
    ...
    morse_sequence_create(morse, &sequence);
    MORSE_sdpotrf_Tile_Async(uplo, A, temp, diag_thick, sequence, &request);
 ...
 *
 ******************************************************************************/
int MORSE_sdpotrf_Tile_Async(MORSE_enum uplo, MORSE_desc_t *A, MORSE_desc_t *temp, int diag_thick,
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    MORSE_context_t *morse;
    void morse_psdpotrf(MORSE_enum, MORSE_desc_t*, MORSE_desc_t*, int, MORSE_sequence_t*, MORSE_request_t*);
...
/*
    if (chameleon_max(N, 0) == 0)
        return MORSE_SUCCESS;
*/
    morse_psdpotrf(uplo, A, temp, diag_thick, sequence, request);
