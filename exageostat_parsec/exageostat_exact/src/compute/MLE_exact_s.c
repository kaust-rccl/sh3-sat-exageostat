 ...
 * */
{
	...
	//Generate the co-variance matrix C
        VERBOSE("Initializing Covariance Matrix (Synthetic Dataset Generation Phase - single precision) .....");
       	MORSE_MLE_scmg_Tile_Async(MorseLower, data->descC, msequence, mrequest, &data->l1, &data->l1, initial_theta, data->dm, data->c_fun);
       	VERBOSE(" Done.\n");
...
//****************************
	...
	MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC, msequence, &mrequest[0], &data->l1, &data->l1, (double *)theta,  data->dm, data->c_fun);    
        MORSE_Sequence_Wait(msequence);
