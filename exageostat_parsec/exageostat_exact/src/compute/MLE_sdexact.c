...
void MORSE_MLE_sdregister_Tile(MLE_data *data)
{
	...
	void MORSE_MLE_sdmat_reg_Tile_Async(MORSE_enum uplo, MORSE_desc_t *, MORSE_sequence_t *, MORSE_request_t  *);
...
double MORSE_sdmle_Tile(unsigned n, const double * theta, double * grad, void * MORSE_data) {
	...
	MORSE_MLE_sdcmg_Tile_Async(MorseLower, MORSE_descC, msequence, &mrequest[0], &data->l1, &data->l1, (double *)theta,  data->dm, diag_thick, data->c_fun);    
	MORSE_Sequence_Wait(msequence);
...
double MORSE_sdmle_Tile_Async(unsigned n, const double * theta, double * grad, void * MORSE_data) {
	...
	VERBOSE("Generate New Covariance Matrix (single-double precision)...");
	START_TIMING(matrix_gen_time);	
	MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC, msequence, mrequest, &data->l1, &data->l1, (double*) theta, data->dm, data->c_fun);    
	MORSE_Sequence_Wait(msequence);
	STOP_TIMING(matrix_gen_time);
	VERBOSE(" Done.\n");
	 ...
	 * */
{
	...
	MORSE_MLE_sdcmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest, &data->lobs, &data->lobs, theta, data->dm, diag_thick, data->c_fun);
	MORSE_Sequence_Wait(msequence);
	...
	MORSE_MLE_sdcmg_Tile_Async(MorseLower, MORSE_descC12, msequence, mrequest, &data->lmiss, &data->lobs, theta, data->dm, diag_thick, data->c_fun);	
	MORSE_Sequence_Wait(msequence);
	...
	if(MORSE_My_Mpi_Rank() == 0)
	{
		...
		printf("precition mean square error: %f\n", data->mserror);
	 ...
	 * */
{
	...
	//Generate C22 covariance matrix.
	VERBOSE("Generate C22 Covariance Matrix... (Prediction Stage - single-double precision)");
	MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC22, msequence, mrequest,  &data->lobs, &data->lobs, theta, data->dm, data->c_fun);
	VERBOSE(" Done.\n");


	//Generate C12 covariance matrix.
	VERBOSE("Generate C12 Covariance Matrix... (Prediction Stage - single-double precision)");
	MORSE_MLE_scmg_Tile_Async(MorseLower, MORSE_descC12, msequence, mrequest,  &data->lmiss, &data->lobs, theta, data->dm, data->c_fun);
	MORSE_Sequence_Wait(msequence);
	VERBOSE(" Done.\n");
	STOP_TIMING(mat_gen_time);
