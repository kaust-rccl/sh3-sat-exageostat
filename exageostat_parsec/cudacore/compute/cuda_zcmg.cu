...
__global__ void dcmg_array_kernel_3d(double *A, int m, int n, int m0,
		int n0, double* l1_x_cuda, double* l1_y_cuda, double* l1_z_cuda, double* l2_x_cuda, double* l2_y_cuda,
		double* l2_z_cuda, double localtheta0, double localtheta1, double localtheta2, int distance_metric)
{
	...
	expr = sqrt(pow((l2_x_cuda[idx] - l1_x_cuda[idy]), 2) +
			pow((l2_y_cuda[idx] - l1_y_cuda[idy]), 2)+ pow((l2_z_cuda[idx] - l1_z_cuda[idy]), 2));

	expr1 = pow(expr, localtheta2);

	if(expr == 0)
		A[idx + idy * m] = sigma_square;// +tmp; /*+ 1e-4*/;
	else
		A[idx + idy * m] = sigma_square *  exp(-(expr1/localtheta1)); // power-exp kernel
}

void dcmg_array_3d( double *A, int m, int n, int m0,
		int n0, double* l1_x_cuda, double* l1_y_cuda, double* l1_z_cuda,double* l2_x_cuda, double* l2_y_cuda,
		double* l2_z_cuda, double *localtheta, int distance_metric, cudaStream_t stream){
	...
	dcmg_array_kernel_3d<<<dimGrid,dimBlock,0,stream>>>(A, m, n, m0, n0, l1_x_cuda, l1_y_cuda, l1_z_cuda,
			l2_x_cuda, l2_y_cuda, l2_z_cuda, localtheta[0],localtheta[1],localtheta[2], distance_metric);
