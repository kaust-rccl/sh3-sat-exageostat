	 ...
	 * */
{
	...
	//TODO add as a paramter for the function
	data.c_fun="matern";
...
void  mle_general(int *n,  int *ncores, int *gpus, int *ts, int *p_grid, int *q_grid,  double *x, int *xlen, double *y, int *ylen, double *z, int *zlen, double *clb, int *clblen, double *cub, int *cublen,  int *computation,  int *diag_thick, int *lr_acc, int *lr_maxrank,  int *dmetric, double*opt_tol, int *opt_max_iters, double *globalthetaout)
	 ...
	 * */
{
	...
	data.iter_count        = 0;
	data.log        = log;
	data.precision         = 0;   //should be modified to get precision as input
	...
	print_summary(1, *n, *ncores, *gpus, *ts, *ts,  data.computation, 1, 1, 1, data.precision);
...
{
	int comp_mode = 0;
	mle_general(n, ncores, gpus, ts, p_grid, q_grid,  x, xlen, y, ylen, z, zlen, clb, clblen, cub, cublen, &comp_mode,  0, 0, 0,  dmetric, opt_tol, opt_max_iters, globalthetaout);
}
...
{
	int comp_mode = 1;
	mle_general(n, ncores, gpus, ts, p_grid, q_grid,  x, xlen, y, ylen, z, zlen, clb, clblen, cub, cublen, &comp_mode,  0, tlr_acc, tlr_maxrank,  dmetric, opt_tol, opt_max_iters, globalthetaout);
}
...
{
	int comp_mode = 2;
	mle_general(n, ncores, gpus, ts, p_grid, q_grid,  x, xlen, y, ylen, z, zlen, clb, clblen, cub, cublen, &comp_mode,  dst_thick, 0, 0,  dmetric, opt_tol, opt_max_iters, globalthetaout);
}
